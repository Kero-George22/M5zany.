...
public class PricingDAO {
    ...
    public int insert(PricingHistory ph) {
        String sql = "INSERT INTO pricing_history (product_id, branch_id, quantity, wholesale_price, " +
                "suggested_price, suggested_at) VALUES (?, ?, ?, ?, ?, NOW())";
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            System.err.println("SQL ERROR in PricingDAO.insert: " + e.getMessage());
            e.printStackTrace();
        }
        return -1;
    }
    ...
    public List<PricingHistory> findAll() {
        ...
        String sql = "SELECT ph.*, p.name as product_name FROM pricing_history ph " +
                "JOIN products p ON ph.product_id = p.id ORDER BY ph.suggested_at DESC";
        try (Connection conn = DatabaseConnection.getConnection();
    ...
    public List<PricingHistory> findByProduct(int productId) {
        List<PricingHistory> list = new ArrayList<>();
        String sql = "SELECT ph.*, p.name as product_name FROM pricing_history ph " +
                "JOIN products p ON ph.product_id = p.id WHERE ph.product_id = ? ORDER BY ph.suggested_at DESC";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
    ...
    private PricingHistory mapRow(ResultSet rs) throws SQLException {
        ...
        ph.setWholesalePrice(rs.getDouble("wholesale_price"));
        ph.setSuggestedPrice(rs.getDouble("suggested_price"));
