...
public class ProductDAO {
    ...
    /**
     * Finds a product by barcode ONLY if it exists in the given branch's inventory.
     * Used for Cashier mode — a cashier should not see products from other branches.
     */
    public Product findByBarcodeInBranch(String barcode, int branchId) {
    ...
    /** Same as findByBarcodeInBranch but searches by product ID. */
    public Product findByIdInBranch(int productId, int branchId) {
        String sql = "SELECT p.*, i.quantity, i.min_stock " +
                "FROM products p INNER JOIN inventory i ON p.id = i.product_id " +
    ...
    public Product findByName(String name) {
        String sql = "SELECT p.*, COALESCE(i.quantity, 0) as quantity, COALESCE(i.min_stock, 10) as min_stock " +
                "FROM products p LEFT JOIN inventory i ON p.id = i.product_id AND p.branch_id = i.branch_id WHERE p.name = ?";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
